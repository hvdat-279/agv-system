...
class Nav2Client:
    ...
    def send_goal(self, x, y, yaw):
        self.node.get_logger().info(f"Nav2Client: Sending goal (x={x}, y={y}, yaw={yaw})")
        self.goal_done = False
        self.goal_success = False
    ...
    def get_result_callback(self, future):
        ...
        if status == 4:
            ...
            self.node.get_logger().info("Nav2Client: Goal reached successfully!")
        else:
            self.goal_success = False
            self.node.get_logger().error(f"Nav2Client: Goal failed with status: {status}")
        self.goal_done = True
